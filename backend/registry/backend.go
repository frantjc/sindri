...
func (b *Registry) Root(context.Context) (http.Handler, error) {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		req, err := http.NewRequestWithContext(r.Context(), r.Method, b.getURL().JoinPath("/v2/").String(), nil)
		if err != nil {
...
func (b *Registry) Token(context.Context) (http.Handler, error) {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		req, err := http.NewRequestWithContext(r.Context(), r.Method, fmt.Sprintf("%s?%s", b.getTokenURL().String(), q.Encode()), nil)
		if err != nil {
			log.Error(err.Error())
			http.Error(w, err.Error(), httputil.HTTPStatusCode(err))
			return
...
func (b *Registry) proxy(name, api string, reference digest.Digest) (http.Handler, error) {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		req, err := http.NewRequestWithContext(r.Context(), r.Method, b.getURL("/v2", b.Repository, name, api, reference.String()).String(), nil)
		if err != nil {
			log.Error(err.Error())
			http.Error(w, err.Error(), httputil.HTTPStatusCode(err))
			return
